pipeline {
    ...
    stages {
        stage('Set_environment_variables'){
            steps{
                script {
                    withCheck('Set_environment_variables') {
                        sh '''#!/usr/bin/env bash
                        {
                        set +x
        ...
        stage('Initialize'){
            stages{
                stage('Install_automatic_validation_package_HGCTPGValidation') {
                    steps {
                        script {
                            withChecks('Install_automatic_validation_package_HGCTPGValidation') {
                                sh '''#!/usr/bin/env bash
                                {
                                set +x
                ...
                stage('Clean_the_working_environment'){
                    steps{
                        script {
                            withChecks('Clean_the_working_environment') {
                                sh '''#!/usr/bin/env bash
                                {
                                set +x
                ...
                stage('Set_CMSSW_environment_variables'){
                    steps{
                        script {
                            withChecks('Set_CMSSW_environment_variables') {
                                script{
                                    sh '''#!/usr/bin/env bash
                                    {
                                    set +x
                ...
                stage('Set_config_files_for_specific_release'){
                    steps{
                        script {
                            withChecks('Set_config_files_for_specific_release') {
                                sh '''#!/usr/bin/env bash
                                {
                                set +x
                ...
                stage('Update_the_configuration'){
                    ...
                    steps {
                        script {
                            withChecks('Update_the_configuration') {
                                sh '''#!/usr/bin/env bash
                                {
                                set +x
        ...
        stage('Install_CMSSW_Test_release'){
            steps {
                script {
                    withChecks('Install_CMSSW_Test_release') {
                        sh '''#!/usr/bin/env bash
                        {
                        set +x
                ...
                stage('Compare_with_CMSSW_Ref_Release'){
                    stages{
                        stage('Install Ref Release'){
                            steps {
                                script{
                                    withChecks('Compare_with_CMSSW_Ref_Release') {
                                        sh '''#!/usr/bin/env bash
                                        {
                                        set +x
